...
namespace SmartElectronicsApi.Mvc.Areas.AdminArea.Controllers
{
    ...
    public class BrandController : Controller
    {
        ...
        public async Task<IActionResult> Update(int? id, BrandUpdateVM brandUpdateVM)
        {
            ...
            if (!string.IsNullOrEmpty(brandUpdateVM.Name))
            {
                content.Add(new StringContent(brandUpdateVM.Name), "Name");
            }

            if (!string.IsNullOrEmpty(brandUpdateVM.Description))
            {
                content.Add(new StringContent(brandUpdateVM.Description), "Description");
            }
            ...
            if (content.Any())
            {
                var response = await client.PutAsync("Brand", content);
                if (response.IsSuccessStatusCode)
                {
                    return RedirectToAction("Index");
                }
            ...
            {
                ModelState.AddModelError(string.Empty, "No data provided to update.,only image can not be sent null");
            }
